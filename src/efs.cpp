...
private:
  ...
  string name;
public:
  City(string geocode, string name);
  City& operator=(const City& t);
  double getLat();
  double getLng();
  string getNam();  
};
...
City& City::operator=(const City& t) {
  return *this;
}
  ...
  int main()
  {
    ...
    while (Continue == 1)
        ...
        else
	  {
	    ...
	    matplot::title("Total distance " + to_string(data.second) + " miles");
	  }
	...
	while (!airports.empty()) {
	  string aircode = airports.front();
	  string geocode =  g->geocode(aircode);
	  City* departing = new City(geocode, aircode);
	  lat.push_back(departing->getLat());
	  lng.push_back(departing->getLng());
	  nam.push_back(departing->getNam());
	  airports.pop();
	}
	
	
	matplot::geolimits({25,55},{-125, -65});
	
	matplot::geoplot(lat, lng)->marker("o");
	matplot::text(lng, lat, nam);
	matplot::show();
